...
//         ctx.replyWithHTML(`<B>O'ZBEKISTON RESPUBLIKASI QURILISH VAZIRLIGI </B>\n (<b>ШНК</b> - SHaharsozlik Norma va Qoidalar)\n(<b>КМК</b> - Qurilish Me'yorlari va Qoidalari) bilan tanishing`)
    ...
    else if (msg.includes(`арестон`)) {
        return ctx.reply(`Е18-2-3-(1-4) Установка Водоподогревателей (Арестон)`)
    }
        ...
        return ctx.reply(`Е15-2-19-7(1-10) СПЛОШНОЕ ВЫРАВНИВАНИЕ ПОВЕРХНОСТЕЙ (ОДНОСЛОЙНАЯ ШТУКАТУРКА) ГИПСОВЫМИ СУХИМИ СМЕСЯМИ(Ротбанд)\nЕ61 ШТУКАТУРНЫЕ РАБОТЫ\nЕ15-2-1-1 Улучшенная штукатурка стен.\nЕ15-2-31-1 Штукатурка откосов.\nЕ15-2-36-1 Штукатурка по Сетке стен.\nВнутренний отделка.\nE15-2-31-1 Штукатурка откосов.\nЕ15-2-16-5 Штукатурка высококачественные стен.\nE46-2-9-2	ОТБИВКА ШТУКАТУРКИ С ПОВЕРХНОСТЕЙ СТЕН И ПОТОЛКОВ КИРПИЧНЫХ \nЕ15-2-40-1 ШТУКАТУРКА ВЕНЕЦИАНСКАЯ, \nЕ15-2-40-2 НА КАЖДЫЙ ПОСЛЕДУЮЩИЙ СЛОЙ ВЕНЕЦИАНСКОЙ ШТУКАТУРКИ ДОБАВЛЯТЬ К НОРМЕ 15-2-40-1`)
    }
    else if (msg.includes(`травертин`)) {
        return ctx.reply(`Е15-2-39-1 Травертин жидкий без ресунки, \nЕ15-2-39-2 Травертин жидкий с ресунки\nЕ15-1-2-(11-15) Травертин Плиты\nЕ15-1-4-(17-24) Травертин Плиты\nЕ15-1-9-(4-6) Травертин Плиты`)
    }
    else if (msg.includes(`отточенто`)) {
        return ctx.reply(`Е15-4-5-9 Окраска Потолков под Венецианскую окраску\nЕ15-4-6-2 ПОКРЫТИЕ ПОВЕРХНОСТЕЙ ГРУНТОВКОЙ ГЛУБОКОГО ПРОНИКНОВЕНИЯ: ЗА 2 РАЗА ПОТОЛКОВ\nЕ15-4-26-9 ВЫСОКОКАЧЕСТВЕННАЯ ОКРАСКА  ПОТОЛКОВ ДЕКОРАТИВНОЙ КРАСКОЙ ОТТОЧЕНТО/ ВЕНИЦИАНСКАЯ`)
    }
    ...
    else if (msg.includes('маслян')) {
        return ctx.reply(`Е15-4-25-8 УЛУЧШЕННАЯ ОКРАСКА МАСЛЯНЫМИ СОСТАВАМИ ПО ШТУКАТУРКЕ СТЕН(Цокол)\nЕ13-6-3-1 ОЧИСТКА ПОВЕРХНОСТИ ЩЕТКАМИ\nЕ62-7-4(1-6) УЛУЧШЕННАЯ МАСЛЯНАЯ ОКРАСКА РАНЕЕ ОКРАШЕННЫХ СТЕН`)
    }
